...
public class TankShooting : NetworkBehaviour
{
    ...
    void OnJoystickMoveEnd(MovingJoystick move)
    {
        if (!isLocalPlayer)
            return;
        //停止时，角色恢复idle  
        if (move.joystickName == "ShootJoystick" )
        {
            Fire();
        }
    }
    ...
    void OnJoystickMove(MovingJoystick move)
    {
        ...
        if (joyPositionY != 0 || joyPositionX != 0)
        {
            ...
            turret.transform.LookAt(new Vector3(turret.transform.position.x + joyPositionX, turret.transform.position.y, turret.transform.position.z + joyPositionY));
